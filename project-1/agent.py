...
class CreatureAgent(CellAgent):
    ...
    def _move_foraging(self):
        ...
        for _ in range(self.model.max_speed):
            ...
            weights = []
            for c in neighbors:
                ndx = c.coordinate[0] - cx
                ndy = c.coordinate[1] - cy
                ca = self._get_cell_agent(c)
                pheromone = ca.pheromone if ca else 0.0
                # Momentum
                dot = hdx * ndx + hdy * ndy
                mag_h = (hdx**2 + hdy**2) ** 0.5
                mag_n = (ndx**2 + ndy**2) ** 0.5
                momentum = (dot / (mag_h * mag_n) + 1.0)
                # Outward bias
                outward = abs(c.coordinate[0] - nx) + abs(c.coordinate[1] - ny)
                w = (
                    pheromone +
                    self.model.momentum_weight * momentum +
                    self.model.outward_weight * outward +

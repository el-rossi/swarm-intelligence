...
1. **Vary Number of Creatures**
   - Change `creature_num` (e.g., 10, 20, 40, 60, 80)
   - Observe: Food collected, survival rate, time to extinction, congestion effects

2. **Vary Number of Food Clusters**
   - Change `cluster_num` (e.g., 2, 6, 12, 15)
   - Observe: Foraging efficiency, time to food depletion, agent distribution

3. **Vary Cluster Spread**
   - Change `cluster_spread` (e.g., 0.5, 1.5, 3.0, 5.0)
   - Observe: How food spatial distribution affects foraging and survival

4. **Vary Food Distance from Nest**
   - Change `food_distance_min` (e.g., 1, 4, 6, 10)
   - Observe: Impact on energy spent, survival, and food collection

5. **Vary Energy Parameters**
...
9. **Grid Size**
   - Change `width` and `height` (e.g., 20x20, 60x60, 100x100)
